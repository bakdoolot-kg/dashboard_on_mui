...
RouterDir = Path(__file__).parent
...
class FileUpdater(ABC):
    ...
    def update(self) -> None:
        """ Get router and then update contents """
...
class JSXFileCreator(FileCreator):
    ...
    def get_absolute_filename(self) -> Path:
        return self._element.full_path / (self._element.name.title() + ".jsx")

    def _write_file_contents(self):
        self.get_absolute_filename().write_text(
            f'''import styles from "./{self._element.name}.module.css";

const {self._element.name.title()} = () => (
    <div className="{(self._element.name).lower()}">{self._element.name}</div>
)
export default {self._element.name.title()};
            '''.strip())
...
class IndexFileCreator(FileCreator):
    ...
    def get_absolute_filename(self) -> Path:
        return self._element.full_path.parent / "index.js"

    def _write_file_contents(self):
        current_file_contents = self.get_absolute_filename().read_text()

        with open(self.get_absolute_filename(), "a", encoding='utf-8') as f:
            f.write(
                f"""export {{ default as {self._element.name.title()}}} from "./{self._element.name}/{self._element.name.title()}";\n""")
            f.close()
...
#             return
#         self.get_absolute_filename().write_text(
#             f"""export {{default as {self._element.name}}} from "./{self._element.name}/{self._element.name}";"""
#         )
